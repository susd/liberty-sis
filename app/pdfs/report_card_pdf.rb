...
class ReportCardPdf
  ...
  def collate_data
    data = {
      'name'        => @report_card.student.name,
      'year'        => Period.current.year,
      'school'      => @report_card.student.site.name,
      'teacher'     => @report_card.fetch_data(['teacher_name']),
      'principal'   => @report_card.student.site.principal,
    ...
    @report_card.fetch_data['comments'].each do |period, values|
      data['comments'][period] = Comment.find(values['comment_ids']).map(&:english)
    end
    ...
    if @report_card.student.home_lang.name == "Spanish"
      ...
      @report_card.fetch_data['comments'].each do |period, values|
        data['spanish_comments'][period] = Comment.find(values['comment_ids']).map(&:spanish)
      end
  ...
  def tabulate_attendance(lang = :english)
    @att_hsh ||= aeries_student.attendance_by_type
